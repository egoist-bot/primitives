...
function useFocusGuards(ownerDocument = document) {
  React.useEffect(() => {
    ...
    ownerDocument.body.insertAdjacentElement('afterbegin', edgeGuards[0] ?? createFocusGuard(ownerDocument));
    ownerDocument.body.insertAdjacentElement('beforeend', edgeGuards[1] ?? createFocusGuard(ownerDocument));
    count++;
